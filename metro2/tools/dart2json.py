...
OUT = pathlib.Path(__file__).resolve().parents[1] / 'src/data/new_meridian.json'
...
src = SRC.read_text()
block = city_block(src, 'newMeridian')
...
city['districts'] = [
    ...
    for t, x, y in re.findall(r"WaterLabel\('([^']+)', ([\d.]+), ([\d.]+)", 
        re.search(r"districts: \[(.*?)\],", block, re.S).group(1))
]
